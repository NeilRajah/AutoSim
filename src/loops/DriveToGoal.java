...
public class DriveToGoal extends Command {
	...
	/**
	 * Initialize the command by updating the setpoints and setting the state machine state
	 */
	...
	protected boolean isFinished() {
		if (FieldPositioning.isWithinBounds(goalPoint, loop.getRobot().getPoint(), tolerance)) {
			return true;
		} else {
			return false;
		} //if
	...
	private void updateSetpoints() {
		...
		if (dist >= 6) { 
			//distance setpoint
			setpoint = loop.getRobot().getAveragePos() + (reverse ? -dist : dist);
	...
	/**
	 * Calculate the angle change required to face point based on the angle from the robot to the point
	 * @return - required angle change in degrees
	 */
	private double calcDeltaAngle() {
		double pointYaw = FieldPositioning.calcGoalYaw(loop.getRobot().getPoint(), goalPoint); //calculate the yaw to the point
		pointYaw += (reverse ? -Math.signum(pointYaw)*180 : 0); //add on 180 in the opposite direction if going in reverse
